...
class LoRa(object):
    ...
    def transmit(self, packet):
        """" Transmit a packet
        :param packet: list of bytes to transmit
        :return: none"""
        self.set_mode(MODE.SLEEP)
    ...
    def _dio0(self, channel):
        ...
        if self.dio_mapping[0] == 0:
            ...
            raise RuntimeError("wrong dio0 mapping!")

    def _dio1(self, channel):
        raise RuntimeError("DIO1 is not used")

    def _dio2(self, channel):
        raise RuntimeError("DIO2 is not used")

    def _dio3(self, channel):
        raise RuntimeError("DIO3 is not used")
    ...
    def set_spreading_factor(self, spreading_factor):
        ...
        if (spreading_factor < 6):
            spreading_factor = 6
        if (spreading_factor == 6):
            self.set_register(REG.LORA.DETECT_OPTIMIZE, 0xc5)
            self.set_register(REG.LORA.DETECTION_THRESH, 0x0c)
        else:
            self.set_register(REG.LORA.DETECT_OPTIMIZE, 0xc3)
            self.set_register(REG.LORA.DETECTION_THRESH, 0x0a)
        self.set_modem_config_2(spreading_factor=spreading_factor)

    def get_spreading_factor(self):
        d = self.get_modem_config_2()
        return d['spreading_factor']
